...
export default function GroceryHome() {
  ...
  const fetchGroceryProducts = async () => {
    try {
      ...
      // ⚡ FIX: Normalize casing to accept both "Grocery" and "grocery" types cleanly
      const groceryProducts = res.data.filter(
        (item) => item.type?.toLowerCase() === "grocery"
      );
  ...
  return (
    <main className="min-h-screen bg-[#f7f7f7]">
      ...
      {/* CATEGORY SELECTOR BADGE SCROLLBAR */}
      <CategoryCard
        selectedCategory={selectedCategory}
        setSelectedCategory={setSelectedCategory}
      />

      {/* PRODUCTS DISPLAY HUB ARRAY VIEWPORT */}
      {loading ? (
        <div className="max-w-7xl mx-auto px-4 py-10">
          <div className="bg-white rounded-3xl p-10 text-center font-black text-green-600 shadow-sm border border-gray-100">
            Loading grocery products...
          </div>
        </div>
      ) : (
        /* ⚡ NOTE: We pass your fresh search filter straight to our case-proofed ProductCard */
        <ProductCard
          products={products.filter((p) =>
            p.name?.toLowerCase().includes(search.toLowerCase())
          )}
